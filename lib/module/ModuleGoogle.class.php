...
class ModuleGoogle extends Module {
	...
	public function handle(Bot $bot) {
		...
		if (substr($bot->message['text'], 0, 8) == '!google ') {
			...
			Bot::queue('Google ['.substr($bot->message['text'], 8).']: http://google.de/search?q='.rawurlencode(substr($bot->message, 8)).' Ungefähr '.format_numeric($data['responseData']['cursor']['estimatedResultCount']).' Ergebnisse');
