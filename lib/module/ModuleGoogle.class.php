...
class ModuleGoogle extends Module {
	...
	public function handle(Bot $bot) {
		...
		if (substr($bot->message['text'], 0, 8) == '!google ') {
			...
			$bot->queue('Google ['.substr($bot->message['text'], 8).']: http://google.de/search?q='.rawurlencode(substr($bot->message['text'], 8)).' Ungefähr '.number_format($data['responseData']['cursor']['estimatedResultCount'], 0, Core::language()->decimal_point, Core::language()->thousand_separator).' Ergebnisse');
